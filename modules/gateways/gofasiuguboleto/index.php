...
if(!function_exists('gib_file_exists_check') ){ #10
    function gib_file_exists_check($file,$sucess_msg=NULL,$error_msg=NULL){
		$file = gib_whmcs_url('root_dir').$file;
    	if(!file_exists($file)){
			if(!$error_msg){
				...
				$error_msg .= '<br>É necessário instalar o <i>hook</i> que acompanha o módulo para todos os recursos funcionarem. <a style="text-decoration:underline;color:red" target="_blank" href="https://gofas.net/gib/#instalation">Saiba mais </a>&#10138;';
				$error_msg .= '</p>';
				return $error_msg;
			}
			if($error_msg){
...
if(!function_exists('gib_reset_local_version')){
	function gib_reset_local_version(){
        try{
	        Capsule::table('tblconfiguration')->where('setting','=','gib_version')->delete();
	        return 'sucess';
        }
        catch (\Exception $e){
            return $e->getMessage();
...
if(!function_exists('gib_verify_module_updates')){
	function gib_verify_module_updates($page_id,$referer,$module_version){
		...
		if( $available_version_int === $module_version_int ){
            ...
            $message .= '<p>Última verificação '.date('d/m/Y à\s H:i', strtotime($updated_at)).' - <a style="text-decoration:underline;" href="'.gib_whmcs_url('admin_url').'/configgateways.php?manage=gofasiuguboleto&resetversion=gofasiuguboleto#m_gofasiuguboleto">verificar agora</a>.</p>';
		}
		if( $available_version_int > $module_version_int ){
            ...
            $message .= '<p>Última verificação '.date('d/m/Y à\s H:i', strtotime($updated_at)).' - <a style="text-decoration:underline;" href="'.gib_whmcs_url('admin_url').'/configgateways.php?manage=gofasiuguboleto&resetversion=gofasiuguboleto#m_gofasiuguboleto">verificar agora</a>.</p>'; #9
		}
		if( $available_version_int < $module_version_int ){
...
if(!function_exists('gib_check_schedule')){
    function gib_check_schedule(){
        $params = getGatewayVariables('gofasiuguboleto');
        $start_at = substr(preg_replace('/[^\da-z]/i','',$params['verifypaymentsat']),0,4) ?: '0500';
        $max_invoices = $params['maxinvoicespercheck'] ?: '100';
        $total_queue_invoices = Capsule::table('tblinvoices')->where('status','=','Unpaid')->where('paymentmethod','=','gofasiuguboleto')->count();
		foreach( Capsule::table('tbltransientdata')
            ->where('name','=','iugu.Boleto.Charge.Verification')
            ->orderBy('id','desc')
            ->take('1')
            ->get() as $value){
        ...
        if((int)$total_queue_invoices >= 1){
            if(is_array($tbltransientdata) and (int)date('YmdHi') >= (int)$tbltransientdata['next']){
                foreach( Capsule::table('tblinvoices')
                    ->where('status','=','Unpaid')
                    ->where('paymentmethod','=','gofasiuguboleto')
                    ->orderBy('id','asc')
                    ->take($max_invoices)
                ...
                if($queue_invoices){ // <----
                    ...
                    $data = [
                        'name'=>'iugu.Boleto.Charge.Verification',
                        'data'=>json_encode([
                    ...
                    $transientdata = Capsule::table('tbltransientdata')->where('name','=','iugu.Boleto.Charge.Verification')->update($data);
					unset($transientdata);
...
function gib_check_status_updates($vars){
	...
	$params_api = gib_api_connect();
    //<
    $check_schedule = gib_check_schedule();
    ...
    if(is_array($check_schedule)){
	    try {
	    	...
	    	foreach( Capsule::table('tblinvoices')
	    		...
	    		foreach( Capsule::table('gofasiuguboleto')->where('invoice_id','=',$tblinvoices->id )->get(['charge_id']) as $local_boleto){
	    	...
	    	if (!empty($invoices)) {
	    		foreach ($invoices as $key => $value) {
                    ...
                    $add_trans = gib_add_trans($value['user_id'],$value['invoice_id'],$value['paid_amount'],$value['fee'],'gib-'.$params_api['api_mode'].'-'.$value['trans_id'], 'Boleto pago - confirmação via cron job');
...
if(!function_exists('gofasiuguboleto_config')){
    function gofasiuguboleto_config(){
    	if(stripos($_SERVER['REQUEST_URI'], '/configgateways.php')!==false){
    		...
    		if($_REQUEST['resetversion'] === 'gofasiuguboleto'){ #9
                gib_reset_local_version();
                header_remove();
    			header("Location: ".$whmcs_url['admin_url'].'/configgateways.php?manage=gofasiuguboleto#m_gofasiuguboleto',true,303);
    			exit;
            }
    		foreach( Capsule::table('tblconfiguration')
    		->where('setting','=','Version')
    		...
    		$renderize = array(
    			...
    			'separator_3' => array(
    				'Description' => '<b>Confirmação automática de pagamentos</b> - <a href="https://gofas.net/gib/#autoverifypayments" target="_blank" style="text-decoration: underline;">Entenda como funciona</a> &#10138;
                    <br>'.gib_file_exists_check('/includes/hooks/gofasiuguboleto.php',NULL,$error_msg='<span style="color: red;border-left: 2px solid red;padding-left: 5px;">Atenção! Hook não instalado. Para esse recurso funcionar você deve <b>instalar o hook que acompanha o módulo</b> - <a style="text-decoration:underline;color:red" target="_blank" href="https://gofas.net/gib/#instalation">Saiba mais </a>&#10138;.</span>').'
                    ',
                ),
		...
		if(file_exists(__DIR__.'/custom/config.php')){
			...
			if(is_array($gib_custom_config) and is_array($renderize) and is_array($footer)){
				$separator_custom = ['separator_custom' => [
					'Description' => '
						<div class="gib_separator">
							<h4>Configurações personalizadas</h4>
						</div>',
					],
				];
				$gib_config = array_merge($renderize,$separator_custom,$gib_custom_config,$footer);
			}
		}
    	if(!file_exists(__DIR__.'/custom/config.php') || !$gib_custom_config and (is_array($renderize) and is_array($footer))){
    		$gib_config = array_merge($renderize,$footer);
    	}
    	return $gib_config;
    }
}
if(!function_exists('gofasiuguboleto_link')){
    function gofasiuguboleto_link($params){
    	//if(stripos($_SERVER['REQUEST_URI'], 'viewinvoice.php') !== false ){
    		...
    		if($params['amount'] >= $params['minimunamount']){	
    			...
    			$result .= '<input type="hidden" id="system_url" value="'.gib_whmcs_url('whmcs_url').'">';
    			...
    			if($saved_boleto['pdf'] and $saved_boleto_amount === $invoice_int_amount and $billet_duedate_int >= $now_int ){
    				...
    				if((string)$charge_verify['result']['status'] === (string)'paid'){
    					$add_trans = gib_add_trans($params['clientdetails']['id'], $params['invoiceid'], (float)number_format( $charge_verify['result']['total_paid_cents']/100,  2, '.', ''), (float)number_format( $charge_verify['result']['taxes_paid_cents']/100,  2, '.', ''), 'gib-'.$saved_boleto['charge_id'].'-'.$params_api['api_mode'], 'Boleto pago - confirmação ao acessar a fatura');
    					header_remove();
    					header("Location: ".gib_whmcs_url('whmcs_url').'/viewinvoice.php?id='.$params['invoiceid'],true,303);
    					exit;
    				}
